  ...
  return (
    ...
    >
      ...
      <View className="flex-1 pr-3">
        <Text
          className={`font-bold text-sm leading-tight ${isInCart && canAddToCart ? 'text-accent' : 'text-primaryText'}`}
          numberOfLines={1}
        >
          {product.item_name}
        </Text>
        <Text className={`text-[11px] font-bold mt-0.5 ${isInCart && canAddToCart ? 'text-secondary' : 'text-secondaryText'}`}>
          ₹{product.retail_price}
          <Text className="opacity-50"> / {product.unit || 'pcs'}</Text>
        </Text>
      </View>

      {/* ── Right: quantity controls (Only if permitted) ───────────── */}
      {canAddToCart && (
