...
export const createBill = async (req, res) => {
  try {
    const {
      ...
      extra_fare, discount, amount_paid, brand_converted_by
    } = req.body;
    ...
    let itemsTotal = 0;
    items.forEach(item => { itemsTotal += Number(item.sale_price) * Number(item.quantity); });
    const total_amount = Math.max(0, itemsTotal + Number(extra_fare || 0) - Number(discount || 0));
    ...
    if (!is_estimate) {
      ...
      else if (finalAmountPaid > 0)        status = 'Partially Paid';
      else                                 status = 'Unpaid';
    }
...
export const updateBill = async (req, res) => {
  try {
    ...
    let itemsTotal = 0;
    newItems.forEach(item => { itemsTotal += Number(item.sale_price) * Number(item.quantity); });
    ...
    if (!originalBill.is_estimate) {
      newAmountPaid = amount_paid !== undefined
