...
// // ── Settings row ──────────────────────────────────────────────────────────────
// const Row = ({
//   icon, iconBg = 'bg-card', iconColor = '#393f35',
//   label, sub, badge,
//   onPress, rightText, danger,
// }) => (
//   <TouchableOpacity
//     onPress={onPress}
...
//   >
//     <View className={`w-9 h-9 rounded-xl items-center justify-center mr-4 flex-shrink-0 ${iconBg}`}>
//       <Feather name={icon} size={15} color={danger ? '#ef4444' : iconColor} />
//     </View>
//     <View className="flex-1">
//       <View className="flex-row items-center gap-2">
//         <Text className={`font-bold text-[14px] ${danger ? 'text-red-500' : 'text-primaryText'}`}>{label}</Text>
//         {badge && (
//           <View className="bg-accent px-1.5 py-0.5 rounded-md">
//             <Text className="text-primaryText font-black text-[8px] uppercase tracking-widest">{badge}</Text>
//           </View>
//         )}
//       </View>
//       {sub ? <Text className="text-secondaryText text-[10px] font-bold mt-0.5">{sub}</Text> : null}
//     </View>
//     {rightText
//       ? <Text className="text-secondaryText text-[11px] font-bold mr-1">{rightText}</Text>
//       : onPress ? <Feather name="chevron-right" size={15} color="#c8c0b4" /> : null}
//   </TouchableOpacity>
// );
...
//       style={{ shadowColor: accent ? '#1f2617' : '#c8c0b4', shadowOpacity: accent ? 0.2 : 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: accent ? 4 : 2 }}
//     >
//       {children}
//     </View>
//   </View>
...
//         {!isStaff && (
//           <>
//             {/* Reports — visually distinguished with accent rows */}
...
//                 <Row
//                   icon="user" iconBg="bg-green-50" iconColor="#10b981"
//                   label="Profile" sub="Shop name, address & more"
...
//                   <View className="flex-1">
//                     <View className="flex-row items-center gap-2">
//                       <Text className="text-primaryText font-bold text-[14px]">Analytics</Text>
//                       {!profile.isPremium && (
//                         <View className="bg-amber-400 px-1.5 py-0.5 rounded-md">
//                           <Text className="text-primaryText font-black text-[8px] uppercase tracking-widest">Pro</Text>
//                         </View>
//                       )}
//                     </View>
//                     <Text className="text-secondaryText text-[10px] font-bold mt-0.5">Sales, revenue & profit trends</Text>
//                   </View>
//                   <Feather name="chevron-right" size={15} color="#7c3aed" />
//                 </TouchableOpacity>
//                 <Sep />
//                 <Row
//                   icon="file-text" iconBg="bg-blue-50" iconColor="#2563eb"
//                   label="Bills & Invoices" sub="All generated bills"
...
//               <Row
//                 icon="users" iconBg="bg-indigo-50" iconColor="#4338ca"
//                 label="Manage Staff" sub="Roles, accounts & permissions"
//                 onPress={() => router.push('/staff')}
//               />
//               <Sep />
//               <Row
//                 icon="book" iconBg="bg-teal-50" iconColor="#0d9488"
//                 label="Manage Customers" sub="Ledger and customer records"
//                 onPress={() => router.push('/customer')}
//               />
//               <Sep />
//               <Row
//                 icon="package" iconBg="bg-orange-50" iconColor="#ea580c"
//                 label="Bulk Add Products" sub="Quick inventory import"
...
//             <Row
//               icon="file-text" iconBg="bg-blue-50" iconColor="#2563eb"
//               label="Bills" sub="View your recent bills"
...
//               </View>
//               <Text className="text-primaryText font-bold text-[14px] flex-1">Log Out</Text>
...
//                     <Text className="text-secondaryText text-[10px] font-bold mt-0.5">Permanently removes your shop & data</Text>
//                   </View>
//                 </TouchableOpacity>
//               </>
//             )}
...
// ── Settings row ──────────────────────────────────────────────────────────────
const Row = ({ icon, iconBg = 'bg-card', iconColor = '#393f35', label, sub, badge, onPress, rightText, danger }) => (
  <TouchableOpacity
    onPress={onPress}
    activeOpacity={onPress ? 0.7 : 1}
    className="flex-row items-center px-4 py-3.5"
...
const StaffCard = ({ profile }) => {
  ...
  const permLevel = (perms) => {
    ...
    if (read && create && update && del) return { label: 'Full',     color: '#16a34a' };
  ...
  return (
    <View className="px-5 mb-4">
      <View
        ...
        style={{ shadowColor: '#1f2617', shadowOpacity: 0.28, shadowRadius: 16, shadowOffset: { width: 0, height: 6 }, elevation: 6 }}
      >
        {/* Accent top stripe */}
        ...
        <View className="px-5 pt-5 pb-5">
          ...
          <View className="flex-row items-center mb-4">
            <View className="bg-accent w-14 h-14 rounded-2xl items-center justify-center mr-4 flex-shrink-0">
              <Text className="text-primaryText font-black text-xl">{initials}</Text>
            </View>
            <View className="flex-1">
              <Text className="text-accent font-black text-[18px] leading-tight" numberOfLines={1}>
                {profile.name}
              </Text>
            ...
            <View className={`px-3 py-1.5 rounded-xl border ml-2 ${isSuspended ? 'bg-red-500/20 border-red-400/30' : 'bg-accent/15 border-accent/20'}`}>
              <Text className={`text-[9px] font-black uppercase tracking-widest ${isSuspended ? 'text-red-400' : 'text-accent'}`}>
                ...
                {Object.entries(profile.permissions).map(([mod, perms]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text className="text-secondary text-[10px] font-bold capitalize opacity-80">{mod}</Text>
                      <Text className="text-[9px] font-black uppercase" style={{ color }}>{label}</Text>
                    </View>
                  );
                })}
              </View>
...
export default function ProfileScreen() {
  const router = useRouter();
  const { profile, loading, isStaff, handleLogout, handleDeleteAccount } = useProfile();
      ...
      >
          ...
          <>
            ...
            <View className="mb-4 px-5">
              ...
              >
                <Row
                  icon="user" iconBg="bg-green-50" iconColor="#10b981"
                  label="Profile" sub="Name, business details & plan"
                  onPress={() => router.push('/account')}
                />
                <Sep />
        ...
        {isStaff && (
          <>
            ...
            <Group label="Quick Access">
              ...
              <Row
                icon="book" iconBg="bg-teal-50" iconColor="#0d9488"
                label="Customers & Khata" sub="Customer list & outstanding balance"
                onPress={() => router.push('/customer')}
              />
            </Group>
          </>
        ...
        <View className="px-5">
          ...
          <View
            ...
            style={{ shadowColor: '#c8c0b4', shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 2 }}
          >
            <TouchableOpacity onPress={handleLogout} activeOpacity={0.7} className="flex-row items-center px-4 py-3.5">
              <View className="w-9 h-9 bg-card rounded-xl items-center justify-center mr-4">
                <Feather name="log-out" size={15} color="#393f35" />
              </View>
              <Text className="text-primaryText font-bold text-[14px] flex-1">Log Out</Text>
            ...
            {!isStaff && (
              <>
                <Sep />
                <TouchableOpacity onPress={handleDeleteAccount} activeOpacity={0.7} className="flex-row items-center px-4 py-3.5">
                  <View className="w-9 h-9 bg-red-50 rounded-xl items-center justify-center mr-4">
                    <Feather name="trash-2" size={15} color="#ef4444" />
                  </View>
                  <View className="flex-1">
