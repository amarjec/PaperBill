    ...
    <SafeAreaView className="flex-1 bg-bg">
      ...
      <View className="px-6 py-4 flex-row justify-between items-center">
        ...
        <Pressable
          ...
          className={`p-3 rounded-2xl active:opacity-50 ${totalDebt > 0 ? "bg-[#4ade80]/20" : "bg-card"}`}
        >
          <Feather
            name="share-2"
            size={18}
      ...
      >
        ...
        <View className="px-6 mt-2 mb-8">
          <View className="bg-white p-6 rounded-[32px] border border-card shadow-sm items-center">
            ...
            <Text className="text-primaryText font-black text-2xl mb-1">
              {customer.name}
            </Text>
            <Text className="text-secondaryText font-bold text-xs mb-6">
              {customer.phone || "No Phone Number"}
            </Text>

            <View className="bg-red-500/10 w-full p-5 rounded-3xl border border-red-500/20 items-center">
              <Text className="text-red-600/80 text-[10px] uppercase font-bold tracking-widest mb-1">
                Total Outstanding Due
              </Text>
              <Text className="text-red-500 font-black text-4xl">
                ₹{totalDebt}
              </Text>
            </View>
            ...
            {totalDebt > 0 && (
              <Pressable
                onPress={handleShareReminder}
                className="mt-4 flex-row items-center bg-[#25D366]/10 px-4 py-2 rounded-xl border border-[#25D366]/20 active:opacity-50"
              >
                <MaterialCommunityIcons
                  ...
                  color="#25D366"
                />
                <Text className="text-[#25D366] font-bold text-xs uppercase tracking-widest ml-2">
        ...
        <View className="px-6 mb-8">
          <Text className="text-secondaryText text-[10px] font-bold uppercase tracking-widest mb-4 ml-2">
                ...
                <View key={idx} className="flex-row items-center mb-4">
                  ...
                  <View className="flex-1 bg-white p-4 rounded-2xl border border-card shadow-sm flex-row justify-between items-center">
                    ...
                    <View className="items-end">
                      <Text className="text-secondaryText text-[9px] uppercase font-bold tracking-widest mb-0.5">
                        Collected By
                      </Text>
                      <Text className="text-primaryText font-bold text-xs">
                        {txn.received_by || "Owner"}
                      </Text>
                    </View>
                  </View>
                </View>
          ...
          {allBills.length === 0 ? (
            <View className="bg-card/30 p-6 rounded-3xl items-center border border-dashed border-secondary/20">
              <Feather
                ...
                className="mb-2"
              />
              <Text className="text-secondaryText font-bold">
                No invoices found.
              </Text>
            </View>
          ) : (
              ...
              return (
                ...
                >
                  <View className="flex-row justify-between items-start mb-3">
                    ...
                    <View
                      className={`px-2 py-1 rounded border ${getStatusColor(bill.status)}`}
                    >
                      <Text
                        className={`text-[8px] font-black uppercase tracking-widest ${getStatusColor(bill.status).split(" ")[0]}`}
                      >
                        {bill.status}
                      </Text>
                    </View>
                  ...
                  <View className="flex-row justify-between items-end mt-2 pt-3 border-t border-card">
                    <Text
                      className="text-secondaryText font-medium text-[11px] flex-1 pr-4"
                      numberOfLines={1}
                    >
                      Items: {itemsList}
                    </Text>
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-bg px-6 pt-4 pb-8 border-t border-card/50">
        ...
        >
          <MaterialCommunityIcons
            name="cash-multiple"
            size={24}
            color={totalDebt > 0 ? "#e5fc01" : "#bfb5a8"}
          />
          <Text
            className={`font-black text-lg uppercase tracking-widest ml-3 ${totalDebt > 0 ? "text-accent" : "text-secondaryText"}`}
          >
      ...
      <Modal visible={paymentModalVisible} transparent animationType="fade">
        ...
        >
          <View className="bg-bg p-8 rounded-[40px] shadow-2xl items-center">
            ...
            <Text className="text-primaryText text-2xl font-black mb-2 text-center">
              Receive Payment
            </Text>
            <Text className="text-secondaryText font-bold text-xs mb-6 text-center">
              Entering a lump sum will automatically clear the oldest pending
              bills first.
            </Text>
            ...
            <View className="flex-row gap-3 w-full">
              <Pressable
                ...
                className="flex-1 bg-card py-4 rounded-2xl items-center active:opacity-50"
              >
              ...
              <Pressable
                ...
                className="flex-[1.5] bg-[#4ade80] py-4 rounded-2xl items-center active:opacity-50 shadow-lg shadow-green-500/30"
              >
                {isProcessing ? (
                  <ActivityIndicator color="#1f2617" />
                ) : (
                  <Text className="text-[#1f2617] font-black uppercase tracking-widest">
                    Process
                  </Text>
                )}
              </Pressable>
