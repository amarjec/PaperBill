...
export const getDashboardAnalytics = async (req, res) => {
  try {
    ...
    const totals = overallTotals[0] || {
      totalSales: 0, totalReceived: 0,
      totalDiscount: 0, totalExtraFare: 0, totalBills: 0,
    };

    // FIX: Gross item profit overstates because it ignores bill-level discounts and extra fares.
    // Adjust: subtract all discounts applied across bills, add back extra fares collected.
    const grossItemProfit = itemStats.reduce((acc, item) => acc + item.profit, 0);
    const totalProfit     = grossItemProfit - totals.totalDiscount + totals.totalExtraFare;
    ...
    res.status(200).json({
      ...
      topProducts: {
        ...
        byQuantity:  [...itemStats].sort((a, b) => b.qtySold - a.qtySold).slice(0, 5),
      },
      topCustomers,
    });
